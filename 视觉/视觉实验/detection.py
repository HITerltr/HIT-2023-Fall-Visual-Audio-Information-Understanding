...
class Detect(object):
    ...
    def detect(self, batch_idx, conf_preds, decoded_boxes, mask_data, inst_data):
        ...
        if self.use_cross_class_nms:
            ...
            if self.use_cluster_diounms:
                boxes, masks, classes, scores = self.cc_cluster_diounms(boxes, masks, scores, self.nms_thresh,
                                                                        self.top_k)
            if self.use_spm_nms:
                boxes, masks, classes, scores = self.cc_cluster_SPM_nms(boxes, masks, scores, self.nms_thresh,
                                                                        self.top_k)
            if self.use_spm_dist_nms:
                boxes, masks, classes, scores = self.cc_cluster_SPM_dist_nms(boxes, masks, scores, self.nms_thresh,
                                                                             self.top_k)
            if self.use_spm_dist_weighted_nms:
                boxes, masks, classes, scores = self.cc_cluster_SPM_dist_weighted_nms(boxes, masks, scores,
            ...
            if self.use_spm_dist_nms:
                boxes, masks, classes, scores = self.cluster_SPM_dist_nms(boxes, masks, scores, self.nms_thresh,
                                                                          self.top_k)
            if self.use_spm_dist_weighted_nms:
                boxes, masks, classes, scores = self.cluster_SPM_dist_weighted_nms(boxes, masks, scores,
                                                                                   self.nms_thresh, self.top_k)
    ...
    def cc_cluster_nms(self, boxes, masks, scores, iou_threshold: float = 0.5, top_k: int = 200):
        ...
        iou = jaccard(boxes_idx, boxes_idx).triu_(diagonal = 1)
        B = iou
        # 如下所示的for循环修改了之前fast-nms算法中较为简单的直接取maxA方式，从而可以防止fast-nms算法中删除过滤掉的边界框box过多
        for i in range(200):
            A = B
            maxA, _ = torch.max(A, dim = 0)
            E = (maxA <= iou_threshold).float().unsqueeze(1).expand_as(A)
    ...
    def cluster_nms(self, boxes, masks, scores, iou_threshold: float = 0.5, top_k: int = 200,
                    second_threshold: bool = False):
        scores, idx = scores.sort(1, descending=True)
        idx = idx[:, :top_k].contiguous()
        scores = scores[:, :top_k]
    ...
    def cluster_diounms(self, boxes, masks, scores, iou_threshold: float = 0.5, top_k: int = 200,
                        second_threshold: bool = False):
        scores, idx = scores.sort(1, descending=True)
        idx = idx[:, :top_k].contiguous()
        scores = scores[:, :top_k]
    ...
    def cluster_SPM_nms(self, boxes, masks, scores, iou_threshold: float = 0.5, top_k: int = 200,
                        second_threshold: bool = False):
        scores, idx = scores.sort(1, descending=True)
        idx = idx[:, :top_k].contiguous()
        scores = scores[:, :top_k]
    ...
    def cluster_SPM_dist_nms(self, boxes, masks, scores, iou_threshold: float = 0.5, top_k: int = 200,
                             second_threshold: bool = False):
        scores, idx = scores.sort(1, descending=True)
        idx = idx[:, :top_k].contiguous()
        scores = scores[:, :top_k]
    ...
    def cluster_SPM_dist_weighted_nms(self, boxes, masks, scores, iou_threshold: float = 0.5, top_k: int = 200,
                                      second_threshold: bool = False):
        scores, idx = scores.sort(1, descending=True)
        idx = idx[:, :top_k].contiguous()
        scores = scores[:, :top_k]
        ...
        B = iou.mul(E)
        _, n = scores.size()
        weights = (B * (B > 0.8).float() + torch.eye(n).cuda().expand(80, n, n)) * (
            scores.unsqueeze(2).expand(80, n, n))
